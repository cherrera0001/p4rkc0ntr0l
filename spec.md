...
| AC-SEG-2 | **El limitador de intentos de `/api/login` corta de verdad, y su identificador de cliente no lo escribe quien ataca.** `x-forwarded-for` no se lee: un proxy agrega al final, así que su primer elemento es texto del cliente y rotarlo fabricaba un cupo por petición. Se leen sólo cabeceras que escribe el borde —`cf-connecting-ip`, `x-vercel-forwarded-for`, `x-real-ip`— y sin ninguna se cae a clave fija: **limitar de más es preferible a no limitar**. Se mide **contra la URL viva**, con un email distinto por intento, porque el limitador tiene dos claves (`ip:` y `email:`) y repetir el email daría un PASS falso. **Hoy PASS por Cloudflare y FAIL por el camino directo `*.vercel.app`** — eso es M-10, causa sin diagnosticar. | `npm run verificar:seg2 -- <url>` → todas las comprobaciones PASS | universal |
| AC-SECRET-1 | **El árbol versionado no publica secretos ni rutas de la máquina.** Ni tokens, ni claves de API, ni llaves privadas, ni cadenas de conexión con contraseña, ni rutas absolutas con perfil de usuario. Pesa por dos hechos, no por prudencia genérica: **el repositorio es público** y **trata dato personal**. Escanea lo que `git ls-files` versiona —no el directorio—, porque lo que importa es lo que se publica. Un hallazgo que **se ve como fixture** (`.invalid`, `CLAVE_DE_PRUEBA`, interpolación) no cuenta: es `§12`/`CLAUDE.md` §3 usado como discriminante, y evita que el criterio grite sobre las pruebas de `redactarSecretos` hasta que alguien lo apague. Toda excepción va **declarada con su motivo** en el propio script. Viene de `docs/guia-2026-08-20-traduccion.md` §5. | `npm run verificar:secretos` → todas las comprobaciones PASS | universal |
